...
class PythonApi {
  ...
  async saveTempFile(fileName: string, data: number[]) {
    // 将字节数组转为 base64
    const uint8Array = new Uint8Array(data);
    const base64 = btoa(String.fromCharCode(...uint8Array));
    const result = await this.request('POST', '/sftp/save-temp-file', { file_name: fileName, data: base64 });
    return result.path;
  }

  // ==================== 文件安全分析 ====================

  async sftpFileAnalysis(path: string) {
    return this.request('POST', '/file-analysis', { path });
  }

  async sftpFileAnalysisIndependent(path: string) {
    return this.request('POST', '/file-analysis/independent', { path });
  }
  ...
  async sshCreateTerminalSession(terminalId: string, cols: number, rows: number) {
    return this.request('POST', '/ssh/terminal/create', { terminal_id: terminalId, cols, rows });
  }
