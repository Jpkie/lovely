...
async function syncAIServiceConfigFromSettings(): Promise<void> {
  try {
    const settingsContent = await invoke('read_settings_file') as string;
    if (!settingsContent) return;
    const settings = JSON.parse(settingsContent);
    const ai = settings?.ai;
    const currentProviderKey = ai?.currentProvider;
    const providerConfig = currentProviderKey ? ai?.providers?.[currentProviderKey] : null;
    if (!providerConfig || !providerConfig.apiKey) return;

    aiService.saveConfig({
      provider: mapProviderKeyToTypeForChat(currentProviderKey),
      apiKey: providerConfig.apiKey,
      baseUrl: providerConfig.baseUrl || undefined,
      model: providerConfig.model || undefined,
    });
  } catch (error) {
    console.warn('⚠️ 同步 AI 配置到聊天页失败:', error);
  }
}

function getAIChatModelOptions(provider: string): string[] {
  switch (provider) {
    case 'openai':
      return ['gpt-5-mini', 'gpt-4o-mini', 'gpt-4.1-mini', 'gpt-4o'];
    case 'deepseek':
      return ['deepseek-chat', 'deepseek-reasoner'];
    case 'claude':
      return ['claude-3-5-sonnet-20241022', 'claude-3-7-sonnet-latest'];
    default:
      return ['custom-model'];
  }
}
...
function setupGlobalModalFunctions(app: LovelyResApp) {
                ...
                if (currentState?.serverInfo) {
                  app.getStateManager().setState({
                    serverInfo: {
                      ...
                      detailedInfo: sshManager.getSystemInfo()?.detailedInfo,
                    }
                  });
                }
  ...
  function escapeAIChatHtml(text: string): string {
    const div = document.createElement('div');
    div.textContent = text;
    return div.innerHTML;
  }

  function renderAIChatMessages(): void {
    ...
    if (aiChatHistory.length === 0) {
      messagesEl.innerHTML = `
        <div style="color: var(--text-secondary); font-size: 13px;">
          你好，我是 AI 助手。你可以直接问 Linux 运维、安全排查、命令解释等问题。
        </div>
      `;
      return;
    }

    messagesEl.innerHTML = aiChatHistory.map((m) => {
      const align = m.role === 'user' ? 'flex-end' : 'flex-start';
      const bg = m.role === 'user' ? 'var(--primary-color)' : 'var(--bg-secondary)';
      const color = m.role === 'user' ? '#fff' : 'var(--text-primary)';
      const roleLabel = m.role === 'user' ? '你' : 'AI';
      return `
        <div style="display:flex; justify-content:${align};">
          <div style="max-width:80%; padding:10px 12px; border-radius:10px; background:${bg}; color:${color}; border:1px solid var(--border-color);">
            <div style="font-size:11px; opacity:0.8; margin-bottom:4px;">${roleLabel} · ${m.timestamp}</div>
            <div style="white-space:pre-wrap; word-break:break-word;">${escapeAIChatHtml(m.content)}</div>
          </div>
        </div>
      `;
    }).join('');
  ...
  (window as any).initAIChatPage = async function () {
    const statusEl = document.getElementById('ai-chat-provider-status');
    ...
    await syncAIServiceConfigFromSettings();

    const config = aiService.getConfig();
    if (!aiService.isConfigured() || !config) {
      if (statusEl) statusEl.textContent = '未配置 AI，请先到设置页配置';
      if (modelSelect) {
        modelSelect.innerHTML = '<option value="">未配置AI</option>';
        modelSelect.disabled = true;
      }
    } else if (statusEl) {
      statusEl.textContent = `已连接配置：${config.provider}${config.model ? ` / ${config.model}` : ''}`;
      if (modelSelect) {
        const models = getAIChatModelOptions(config.provider);
        const currentModel = config.model || models[0];
        const allModels = models.includes(currentModel) ? models : [currentModel, ...models];
    ...
    if (inputEl && !inputEl.dataset.bound) {
      ...
      inputEl.addEventListener('keydown', (e: KeyboardEvent) => {
        if (e.key === 'Enter' && !e.shiftKey) {
          e.preventDefault();
          (window as any).sendAIChatMessage?.();
        }
      });
    }
  ...
  (window as any).openAISettingsFromChat = function () {
    (window as any).showSettingsOverlay?.();
    setTimeout(() => {
      (window as any).switchSettingsTab?.('ai');
    }, 120);
  };

  (window as any).switchAIChatModel = function (model: string) {
    const config = aiService.getConfig();
    ...
    const statusEl = document.getElementById('ai-chat-provider-status');
    if (statusEl) {
      statusEl.textContent = `已连接配置：${config.provider} / ${model}`;
    }
  };

  (window as any).clearAIChatMessages = function () {
    aiChatHistory = [];
    renderAIChatMessages();
  };

  (window as any).sendAIChatMessage = async function () {
    const inputEl = document.getElementById('ai-chat-input') as HTMLTextAreaElement | null;
    ...
    if (!aiService.isConfigured()) {
      aiChatHistory.push({
        role: 'assistant',
        content: 'AI 未配置。请先到设置页配置 API Key 和模型。',
    ...
    const assistantMessage: AIChatHistoryItem = {
      role: 'assistant',
      content: '',
      timestamp: new Date().toLocaleTimeString(),
    };
    ...
    try {
      const contextMessages = aiChatHistory
        .slice(-12)
        .map((m) => ({ role: m.role as 'user' | 'assistant', content: m.content }));

      await aiService.chatStream(
        [
          { role: 'system', content: '你是 Linux 应急响应与运维助手。请使用中文，给出准确、可执行的建议。' },
          ...contextMessages,
        ],
        (chunk: string) => {
        ...
        (fullText: string) => {
          assistantMessage.content = fullText || assistantMessage.content || '（无内容返回）';
          renderAIChatMessages();
        }
      );
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      assistantMessage.content = `请求失败：${msg}`;
      renderAIChatMessages();
    } finally {
  ...
  // 继续 Agent 调查
  (window as any).continueAgentInvestigation = async function () {
...
(window as any).showSettingsOverlay = function () {
  ...
  // 兜底：立即初始化基础交互，避免异步初始化失败时无法切换AI页
  const fallbackSwitchSettingsTab = (tabName: 'basic' | 'ai') => {
    const scope = document.getElementById('settings-overlay-container') || document;
    scope.querySelectorAll('.settings-tab').forEach((tab) => {
      const tabEl = tab as HTMLElement;
      const isActive = tabEl.getAttribute('data-tab') === tabName;
      tabEl.style.color = isActive ? 'var(--text-primary)' : 'var(--text-secondary)';
      tabEl.style.fontWeight = isActive ? '500' : '400';
      tabEl.style.borderBottom = isActive ? '2px solid var(--accent-color)' : '2px solid transparent';
    });
    scope.querySelectorAll('.settings-panel').forEach((panel) => {
      const panelEl = panel as HTMLElement;
      panelEl.style.display = panelEl.id === `${tabName}-settings` ? 'block' : 'none';
    });
  };
  (window as any).switchSettingsTab = fallbackSwitchSettingsTab;
  (settingsOverlay.querySelectorAll('.settings-tab') || []).forEach((btn) => {
    btn.addEventListener('click', () => {
      const tabName = (btn as HTMLElement).getAttribute('data-tab') as 'basic' | 'ai' | null;
      if (tabName) fallbackSwitchSettingsTab(tabName);
    });
  });

  // 兜底：先填充默认字体，避免一直停留“正在加载字体...”
  const fontSelect = settingsOverlay.querySelector('#global-font') as HTMLSelectElement | null;
  if (fontSelect && fontSelect.options.length <= 1) {
    const defaultFonts = ['系统默认', 'Microsoft YaHei', '微软雅黑', 'SimSun', '宋体', 'Consolas', 'JetBrains Mono'];
    fontSelect.innerHTML = '';
    defaultFonts.forEach((font) => {
      const option = document.createElement('option');
      option.value = font === '系统默认' ? 'system' : font;
      option.textContent = font;
      if (font !== '系统默认') option.style.fontFamily = font;
